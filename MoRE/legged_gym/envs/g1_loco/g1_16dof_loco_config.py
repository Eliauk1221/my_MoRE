...
class G1_16Dof_Loco_Cfg( LeggedRobotCfg ):
    ...
    class terrain:
        ...
        terrain_dict = {"stepping_stones": 1, 
                        "parkour": 1,
                        "pit": 1,
                        "gap": 1,
                        "stair": 1,}
    ...
    class terrain_attention:
        ...
        terrain_attn_query_with_depth = True  # 是否将 depth_feature 拼入注意力 Query（跨模态中期融合）
        ...
        # ===== TerrainSafetyScorer 先验分量权重（消融用） =====
        scorer_w_support = 0.3           # S_support 权重
        scorer_w_margin = 0.3            # S_margin 权重
        scorer_w_edge = 0.5              # edge_bonus 权重，设 0 可禁用（A3 实验）
        scorer_w_forward = 0.8           # forward_bias 权重，设 0 可禁用（A2 实验）
...
class G1_16Dof_Loco_CfgPPO( LeggedRobotCfgPPO ):
    ...
    class policy:
        ...
        terrain_attn_use_pre_ln = True   # Pre-LN: 对 Q/KV 做 LayerNorm，修正 CNN+xyz 的尺度不对称
        ...
        terrain_attn_query_with_depth = True    # True: 将 depth_feature 拼入注意力 Query
