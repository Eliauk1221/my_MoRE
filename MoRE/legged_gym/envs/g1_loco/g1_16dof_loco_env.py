...
class G1_16Dof_Loco_Robot(LeggedRobot):
    ...
    def _reward_feet_safe_landing(self):
        """
        触地时足底采样点局部平坦度奖励。
        
        利用 feet_indicator_pos (num_envs, 2, 6, 3) 中每只脚的 6 个世界坐标点，
        查询高度场得到地形高度，用 6 点高度方差衡量局部平坦度。
        方差越小越平坦，奖励越高。仅在脚初次触地时给予。
        """
        if self.cfg.terrain.mesh_type == 'plane':
            return torch.zeros(self.num_envs, device=self.device)
        
        # feet_indicator_pos: (num_envs, 2_feet, 6_dots, 3_xyz)
        indicator_world = self.feet_indicator_pos  # (N, 2, 6, 3)
        N, num_feet, num_dots, _ = indicator_world.shape
        
        # 转换为高度场像素坐标
        pts_flat = indicator_world.reshape(N * num_feet * num_dots, 3)
        px = ((pts_flat[:, 0] + self.terrain.cfg.border_size) / self.terrain.cfg.horizontal_scale).long()
        py = ((pts_flat[:, 1] + self.terrain.cfg.border_size) / self.terrain.cfg.horizontal_scale).long()
        px = torch.clip(px, 0, self.height_samples.shape[0] - 2)
        py = torch.clip(py, 0, self.height_samples.shape[1] - 2)
        
        # 查询地形绝对高度（取相邻格点最小值，与 _get_heights 一致）
        h1 = self.height_samples[px, py]
        h2 = self.height_samples[px + 1, py]
        h3 = self.height_samples[px, py + 1]
        terrain_h = torch.min(torch.min(h1, h2), h3).float() * self.terrain.cfg.vertical_scale
        
        # reshape 回 (N, 2, 6)
        terrain_h = terrain_h.reshape(N, num_feet, num_dots)
        
        # 每只脚 6 个点的高度方差
        foot_h_var = torch.var(terrain_h, dim=-1)  # (N, 2)
        
        # 奖励 = exp(-k * var)，k 越大对平坦度要求越高
        k = 500.0
        foot_landing_rew = torch.exp(-k * foot_h_var)  # (N, 2)
        
        # 只在初次触地时给予奖励
        first_contact = (self.feet_air_time > 0.) * self.contact_filt  # (N, 2)
        rew = torch.sum(foot_landing_rew * first_contact, dim=1)  # (N,)
